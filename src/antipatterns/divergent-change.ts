...
export function detectDivergentChange(commits: Commit[]): AntiPatternResult {
  const fileTopicMap = new Map<string, Set<string>>();
  ...
  candidates.sort((a, b) => (b.details.distinctReasons as number) - (a.details.distinctReasons as number));
  ...
  const avgReasons = candidates.reduce((s, c) => s + (c.details.distinctReasons as number), 0) / candidates.length;
