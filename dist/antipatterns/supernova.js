export function detectSupernova(commits) {
    ...
    for (const commit of commits) {
        for (const file of commit.files) {
            ...
            if (commit.date < m.dateRange.first)
                m.dateRange.first = commit.date;
            if (commit.date > m.dateRange.last)
                m.dateRange.last = commit.date;
        }
    }
    ...
    const median = churnValues.length % 2 === 0
    ...
    candidates.sort((a, b) => b.details.churn - a.details.churn);
